...
from cfg import cfg
from core.detect import detect_mode_auto, detect_theme_hint, locate_number_roi
...
from core.templates import ensure_dot_template  # auto-create dot if missing
...
async def process_image_pipeline(file_path: str, mode: str, user_caption: str, user_id: int) -> Tuple[str, Dict]:
    ...
    theme_hint = detect_theme_hint(img_bgr)

    target_num_text = normalize_number_for_mode(user_caption, final_mode)

    (rx, ry, rw, rh), theme_used = locate_number_roi(img_bgr, final_mode, theme_hint)
    roi = (rx, ry, rw, rh)
    roi_crop = img_bgr[ry:ry+rh, rx:rx+rw]

    # Pastikan dot.png ada (auto-learn dari ROI jika belum ada)
    ensure_dot_template(final_mode, theme_used, roi_crop)

    base = inpaint_region(img_bgr, roi)
    composed = compose_number_from_templates(target_num_text, final_mode, theme_used)
    ...
    meta = {"mode": final_mode, "theme": theme_used}
